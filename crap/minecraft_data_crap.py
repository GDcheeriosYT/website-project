...
def player_data(get_players):
    if get_players:
        ...
        print(os.listdir("minecraft"))
        print(os.listdir("minecraft/world"))
        ...
        for player in players:
            e = Player(player['username'], player['avatar'],
                       player['data']['Health'] + player['data']['AbsorptionAmount'], player['data']['foodLevel'],
                       player['data']['XpLevel'], player['data']['Score'], player['data']['Pos'])
            updated_players.append(
                {'username': e.name, 'avatar': e.avatar_url, 'health': int(e.health), 'hunger': give_num(e.hunger),
                 'level': e.level, 'score': give_num(e.xp),
                 'position': f"X:{give_num(e.pos[0])}, Y:{give_num(e.pos[1])}, Z:{give_num(e.pos[2])}"})
        ...
        for player in player_data.items():
            ...
            e = Player(player[1]["username"], player[1]["avatar"],
                       player_game_data['Health'] + player_game_data['AbsorptionAmount'], player_game_data['foodLevel'],
                       player_game_data['XpLevel'], player_game_data['Score'], player_game_data['Pos'])
            players.append(
                {'username': e.name, 'avatar': e.avatar_url, 'health': int(e.health), 'hunger': give_num(e.hunger),
                 'level': e.level, 'score': give_num(e.xp),
                 'position': f"X:{give_num(e.pos[0])}, Y:{give_num(e.pos[1])}, Z:{give_num(e.pos[2])}"})
